...
//Liz added this 8,15
const KeyActionsDisplay = ({ currentSecond }) => {
  const scrollRef = useRef(null);

  const currentKeyActions = useMemo(() => {
    return videoSummaries.map((action) => {
      const start = parseTimestamp(action.Start_Timestamp);
      const end = parseTimestamp(action.End_Timestamp);
  ...
  useEffect(() => {
    const currentKeyActionElement = scrollRef.current.querySelector('.current-key-action');
    if (currentKeyActionElement) {
      currentKeyActionElement.scrollIntoView({ behavior: 'auto', block: 'nearest' });
    }
  }, [currentKeyActions]);

  return (
    <ScrollArea className="h-96 w-full" ref={scrollRef}>
      {currentKeyActions.map((action, index) => (
        <div
          key={index}
          className={`w-full ${action.isCurrent ? 'current-key-action' : ''}`}
        >
          <p className="font-bold">{action.Start_Timestamp} - {action.End_Timestamp}</p>
          <p>{action.key_actions}</p>
        </div>
      ))}
    </ScrollArea>
  );
        ...
        {currentActions.map((action, index) => (
          <div key={index} className={`w-full ${action.isCurrent ? 'current-action' : ''}`}>
            <Collapsible className="space-y-2">
              ...
              <CollapsibleContent className="space-y-2 px-4">
                <p className="text-sm">{action.actions}</p>
              </CollapsibleContent>
            </Collapsible>
          </div>
        ))}
...
const VideoSummaryDisplay = ({ currentSecond }) => {
  ...
  const currentSummaries = useMemo(() => {
    return videoSummaries.map((summary) => {
  ...
  return (
    <ScrollArea className="h-96 w-full" ref={scrollRef}>
      {currentSummaries.map((summary, index) => (
        ...
        >
          <p className="font-bold">{summary.Start_Timestamp} - {summary.End_Timestamp}</p>
          <p>{summary.summary}</p>
        </div>
      ))}
    </ScrollArea>
      ...
      <div className="flex flex-col">
        ...
        <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6">
          <div className="flex w-full gap-4">
            ...
            <div className="grid w-[300px] gap-0 h-[45vh]">
              ...
              <Card className="flex-1 mt-2">
                ...
                <CardContent>
                  <p className="text-sm">
                    A video displayed the driver's point of view on a road.
                  </p>
                </CardContent>
              </Card>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-auto" style={{marginTop:"-12rem"}}>
            <Card>
              <CardHeader>
            ...
            <ActionCard actionData={actionSummary} />
          </div>
        </main>
      </div>
